...
**Status:** Complete
**Release:** `v2.2.0 — Response Actions`

### Planned scope

- [x] Add S3 public-access remediation with pre-change configuration capture.
- [x] Add S3 bucket-policy, ACL, and public-access-block inspection.
- [x] Add a controlled S3 rollback action using captured state.
- [x] Add quarantine security-group creation or reuse logic.
- [x] Add EC2 security-group restoration using original associations captured during isolation.
- [x] Add consistent incident-resource tagging.
- [x] Add idempotency checks so repeated invocations do not create unsafe duplicate changes.
- [x] Expand IAM policy examples for each new action.
- [x] Add unit tests, sample events, deployment documentation, troubleshooting, cost, cleanup, and rollback guidance.
- [x] Cross-reference the relevant S3, EC2, IAM, and containment runbooks.
