...
resource "aws_security_group" "target" {
  ...
  description = "Phase 4 target: no ingress; outbound HTTPS only"
  vpc_id      = aws_vpc.lab.id
...
resource "aws_instance" "target" {
  ...
  user_data = <<-EOT
    ...
    printf '%s\n' 'Phase 4 EC2 isolation lab baseline initialized.' > /var/tmp/aws-ir-practice/baseline.txt
    chmod 0644 /var/tmp/aws-ir-practice/baseline.txt
    systemctl enable --now amazon-ssm-agent 2>/dev/null || true
  EOT

  tags = merge(local.common_tags, {
    Name       = "${var.project_name}-target"
    LabPurpose = "authorized-ec2-compromise-isolation-practice"
  })
...
resource "aws_cloudwatch_event_rule" "simulated_finding" {
  ...
  description = "Authorized Phase 4 EC2 isolation lab finding"
...
resource "aws_lambda_permission" "eventbridge_lab" {
  statement_id  = "AllowEventBridgePhase4Ec2Isolation"
